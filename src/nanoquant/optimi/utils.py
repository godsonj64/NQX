...
def param_groups_weight_decay(model: nn.Module, weight_decay: float = 1e-2,
                              additional_layers: Iterable[str] | None = None) -> list[dict[str, Any]]:
...
def _default_to_triton(params: list[torch.Tensor]) -> tuple[bool, bool]:
    ...
    triton = HAS_TRITON and all(p is None or (
        type(p) in _foreach_supported_types and _triton_kernels_supported_device(p) and torch.is_floating_point(p))
                                for p in params)
    return triton
